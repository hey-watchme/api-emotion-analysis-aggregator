...
## 🆕 最新アップデート (2025-10-26) - Kushinada v2 4感情対応

### Kushinada v2への完全移行
- **感情数変更**: 8感情 → 4感情（neutral, joy, anger, sadness）
- **マッピング削除**: 複雑な8感情マッピングロジックを完全削除
- **データ形式**: 0.0-1.0の範囲のfloat値で感情スコアを保持
- **feature-extractor-v2統合**: Kushinada HuBERT-largeモデルの出力をそのまま処理
- **シンプル化**: 不要な変換処理を削除し、処理速度とデータ品質を向上
- **iOS対応**: iOSダッシュボードアプリで4感情を直接表示
